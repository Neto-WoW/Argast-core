...
public:
    ...
    void SetVisual(Player* player, uint32 visual_id)
    {
        ...
        CharacterDatabase.Query("REPLACE into `mod_weapon_visual_effect` (`item_guid`, `enchant_visual_id`) VALUES ('%u', '%u')", item->GetGUID().GetCounter(), visual_id);
    }
...
public:
    ...
    void GetVisual(Player* player)
    {
        ...
        QueryResult result = CharacterDatabase.Query("SELECT item_guid, enchant_visual_id FROM `mod_weapon_visual_effect` WHERE item_guid IN(SELECT guid FROM item_instance WHERE owner_guid = %u)", player->GetGUID().GetCounter());
